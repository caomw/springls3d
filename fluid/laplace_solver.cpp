...
// ans = x^T * x
static double product( RegularGrid<char>& A, RegularGrid<float>& x, RegularGrid<float>& y, int n ) {
    static double ans;
    ans = 0.0;
#ifdef _OPENMP
...
static void flip( RegularGrid<float>& x, int n ) {
	OPENMP_FOR FOR_EVERY_COMP(n) {
		x[i][j][k] = -x[i][j][k];	
	} END_FOR;
}
...
// r = b - Ax
static void residual( RegularGrid<char>& A, RegularGrid<float>& L, RegularGrid<float>& x, RegularGrid<float>& b, RegularGrid<float>& r, int n ) {
...
static void buildPreconditioner( RegularGrid<double>& P, RegularGrid<float>& L, RegularGrid<char>& A, int n ) {
	clear(P,n);
	double a = 0.25;
	FOR_EVERY_COMP(n) {
		if( A[i][j][k] == FLUID ) {
...
static void applyPreconditioner( RegularGrid<float>& z, RegularGrid<float>& r, RegularGrid<double>& P, RegularGrid<float>& L, RegularGrid<char>& A, int n ) {
#if USE_PRECOND
...
static void conjGrad( RegularGrid<char>& A, RegularGrid<double>& P, RegularGrid<float>& L, RegularGrid<float>& x, RegularGrid<float>& b, int n ) {
	// Pre-allocate Memory
